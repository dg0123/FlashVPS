...
def shell():
    ...
    printer('Recuperando configuración de speedtest.net...', quiet)
    try:
        speedtest = Speedtest()
    except ConfigRetrievalError:
        printer('No se puedo recuperar la configuración de speedtest')
        sys.exit(1)
    ...
    if args.list:
        try:
            ...
            print_('No se puedo recuperar la lista de servidores speedtest ')
            sys.exit(1)
    ...
    printer('Testeando de %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)
    ...
    if not args.mini:
        ...
        printer('Seleccionando el mejor servidor basado en su ping...', quiet)
    ...
    printer('Probando velocidad de descarga', quiet,
            end=('', '\n')[bool(debug)])
    ...
    printer('Probando velocidad de subida', quiet,
            end=('', '\n')[bool(debug)])
    ...
    if args.share:
        printer('Compartir resultado: %s' % results.share(), quiet)
